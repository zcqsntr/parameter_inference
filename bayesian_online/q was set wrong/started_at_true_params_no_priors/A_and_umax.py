...
def TS_square_loss(param_vec, initial_S, Cins , actual_Ns, Cs, C_0s, debug = False):
    predicted_Ns = predict_time_series(param_vec, Ns, Cins, Cs, C_0s)
...
def TS_grad_wrapper(param_vec, i):
    return TS_grad_func(param_vec, initial_N, Cins, actual_Ns, C, C_0)
...
# parameters to learn
param_vec = np.array([-0.1,-0.1, -0.1, -0.1, 2., 2.])

prior_centres = np.array([-0.11, -0.09, -0.11, -0.09, 2.1, 1.9])
prior_sigmas = np.array([1, 1, 1, 1, 2, 2])
...
''' ONLINE'''
'''
for mult in [1,0.1,0.01,0.001,0.0001]:
    ...
    for i in range(500):
        alpha = get_learning_rate(i, 0.05,  0.5, 50) *mult
        Cin = Cins[i,:]
        sol = fullSol[i,:]
        N = sol[:2]
...
'''
    ...
    plt.title('Inferring A and u_max ' + str(mult))
    plt.xlabel('Iteration')
    plt.ylabel('Sum of square loss of the parameters')
    plt.legend()
    plt.savefig('parameter_loss_A' + str(mult) + '.png')
    ...
    plt.legend()
    plt.savefig('N_loss_A_and_umax' + str(mult) + '.png')
'''


'''TIME SERIES'''

new_param_vec = adam(TS_grad_wrapper, param_vec, num_iters = 500)
print(new_param_vec)
