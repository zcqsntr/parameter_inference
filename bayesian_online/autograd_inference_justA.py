...
true_params = np.array([-0.1,-0.1, -0.1, -0.1])
prior_sigmas = np.array([-0.01, -0.01, -0.01, -0.01])
...
def MAP_loss(param_vec, current_S, Cin , next_N, C, C_0, debug = False):
    ...
    priors = gaussian(param_vec, true_params, prior_sigmas) # centre on true params for now
