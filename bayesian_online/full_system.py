...
def square_loss_with_priors(param_vec, current_S, Cin, next_N, debug = False):
    ...
    return  np.sum(np.array(next_N - predicted_N)**2) - 1/weight*np.sum(np.log(priors))
...
grad_func = grad(square_loss_with_priors)
